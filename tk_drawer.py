...
class TkDrawer:
    ...
    # Рисование точки
    def draw_point(self, p, colour="black"):
        self.canvas.create_oval(
            x(p) + 1, y(p) + 1, x(p) - 1, y(p) - 1, fill=colour)
        self.root.update()

    # Рисование линии
    def draw_line(self, p, q, colour="black"):
        self.canvas.create_line(x(p), y(p), x(q), y(q), fill=colour, width=2)
        self.root.update()
